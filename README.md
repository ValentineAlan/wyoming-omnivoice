# OmniVoice for Home Assistant

![Wyoming OmniVoice](assets/icon.svg)

Give Assist an OmniVoice voice, generated locally on your separate NVIDIA server.

**GPU installation requires a separate compatible NVIDIA Docker host.** The current
image uses CUDA 13 and supports Turing-or-newer hardware with a compatible driver.
The Home Assistant companion needs no GPU; it helps you connect that server.

**[Hear the synthetic voice example (WAV)](https://raw.githubusercontent.com/ValentineAlan/wyoming-omnivoice/main/voices/example_reference.wav)**
— generated by OmniVoice without a reference recording. [Transcript and provenance](voices/README.md).
This is an audio example, not a recording of an end-to-end Assist request.

| What would you like to do? | Start here |
| --- | --- |
| Set up OmniVoice for the first time | [Guided setup](docs/quick-start.md) |
| Use a Home Assistant app to help | [Install OmniVoice Connect](https://github.com/ValentineAlan/omnivoice-connect) — experimental |
| Connect an existing server | [Open Wyoming setup](https://my.home-assistant.io/redirect/config_flow_start/?domain=wyoming) |
...
Measured RTX 3090 warm first-audio latency was 0.21–0.25 seconds in a small test
using 12 steps, short chunks, FlashInfer and CUDA graphs; the first request after
restart was about 2.9 seconds. These are TTS server measurements, not full Assist
latency or default-setting guarantees. [Hardware and validation details](VALIDATION.md).
